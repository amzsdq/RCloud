...
async function fetchJson(url, attempts = 3) {
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      const response = await fetch(`${url}${separator}cb=${Date.now()}-${attempt}`, {
        headers: { "user-agent": "RCloud-Queue/0.1", "cache-control": "no-store" }
      });
...
export async function readQueueIndex() {
  return validateQueueIndex(await fetchJson(QUEUE_INDEX_URL));
}
...
export async function drainQueue({ isTerminal, process, limit = QUEUE_BATCH_LIMIT }) {
  ...
  for (const item of index.requests) {
    ...
    if (await isTerminal(item.request_id)) {
      skippedTerminal += 1;
      continue;
    }
