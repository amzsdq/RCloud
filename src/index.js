...
export class RuntimeState extends DurableObject {
  ...
  async getMailboxStatus() { return { last_request_id: (await this.ctx.storage.get("mailbox_last_request_id")) ?? null, receipt: (await this.ctx.storage.get("mailbox_receipt")) ?? null, last_ai_result: (await this.ctx.storage.get("last_ai_result")) ?? null, last_poll: (await this.ctx.storage.get("last_poll")) ?? null, recent_polls: (await this.ctx.storage.get("poll_history")) ?? [] }; }
...
async function pollMailbox(runtime) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      const response = await fetch(MAILBOX_URL, { headers: { "user-agent": "RCloud/0.9", "cache-control": "no-cache" } }); if (!response.ok) throw new Error(`HTTP_${response.status}`);
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/health") return Response.json({ ok: true, service: "RCloud", version: "0.9.0", runtime: "cloudflare-worker+durable-object+alarm+cron+github-mailbox+workers-ai", control_plane: "github-main", ai_model: AI_MODEL, time: new Date().toISOString() });
